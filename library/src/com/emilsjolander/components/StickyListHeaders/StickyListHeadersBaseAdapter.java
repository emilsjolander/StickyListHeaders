...
public abstract class StickyListHeadersBaseAdapter extends BaseAdapter implements StickyListHeadersAdapter{
	...
	private View getHeaderWithForPosition(int position){
		...
		header.setId(R.id.com_emilsjolander_components_stickylistheaders_header_view);
		return header;
	}
	
	//attaches a header to a list item
	private View attachHeaderToListItem(View header, View listItem){
		listItem.setId(R.id.com_emilsjolander_components_stickylistheaders_list_item_view);
	...
	private View attachDividerToListItem(View listItem) {
		...
		if(divider == null){
			...
			divider.setId(R.id.com_emilsjolander_components_stickylistheaders_divider_view);
	...
	private View axtractHeaderAndListItemFromConvertView(View convertView){
		...
		View header = vg.findViewById(R.id.com_emilsjolander_components_stickylistheaders_header_view);
		if(header!=null){
			header.setVisibility(View.VISIBLE);
			headerCache.add(header);
		}
		
		View divider = vg.findViewById(R.id.com_emilsjolander_components_stickylistheaders_divider_view);
		if(divider!=null){
			dividerCache.add(divider);
		}
		
		View listItem = vg.findViewById(R.id.com_emilsjolander_components_stickylistheaders_list_item_view);
		vg.removeAllViews();
